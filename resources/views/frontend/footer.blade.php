<!-- Contact Part Start -->
<section id="contact">
  <div class="container">
    <div class="row">
      <div class="col-lg-3 col-sm-6">
        <div class="cont-content text-center">
          <h5>Need Help?</h5>
          <div class="cont-span">
            <span><a data-toggle="modal" data-target="#login">Member Login</a></span>
            <span><a href="{{ route('register') }}">Sign Up</a></span>
            <span><a href="{{ url('/partner_search_policy') }}">Partner Search</a></span>
            <span><a href="#">How to Use Zibonshathi.com</a></span>
            <span><a href="{{ url('/premium_membership') }}">Premium Memberships</a></span>
            <span><a href="#">Customer Support</a></span>
            <span><a href="#">Site Map</a></span>
          </div>
        </div>
      </div>
      <div class="col-lg-3 col-sm-6">
        <div class="cont-content text-center">
          <h5>Company</h5>
          <div class="cont-span">
            <span><a href="{{ url('/aboutus') }}">About Us</a></span>
            <span><a href="{{ url('/allBlog') }}">Zibonshathi Blog</a></span>
            <span><a href="#">Careers</a></span>
            <span><a href="#">Awards & Recognition</a></span>
            <span><a data-toggle="modal" data-target="#contact">Contact Us</a></span>
          </div>
        </div>
      </div>
      <div class="col-lg-3 col-sm-6">
        <div class="cont-content text-center">
          <h5>Privacy & You</h5>
          <div class="cont-span">
            <span><a href="{{ url('/termsof_us') }}">Terms Of Use</a></span>
            <span><a href="{{ url('/privacy_policy') }}">Privacy Policy</a></span>
            <span><a href="#">Be Safe Online</a></span>
            <span><a href="#">Report Misuse</a></span>
          </div>
        </div>
      </div>
      <div class="col-lg-3 col-sm-6">
        <div class="cont-content text-center">
          <h5>Follow us</h5>
          <div class="cont-spans">
            <a href="#"><i class="fab fa-facebook-f"></i></a>
            <a href="#"><i class="fab fa-twitter"></i></a>
            <a href="#"><i class="fab fa-linkedin-in"></i></a>
            <a href="#"><i class="fab fa-youtube mr-1"></i></a>
          </div>
        </div>
      </div>

    </div>
  </div>
</section>
<!-- Contact Part End -->
